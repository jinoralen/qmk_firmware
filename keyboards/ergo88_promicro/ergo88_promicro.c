...
void config_on(uint8_t addr) {
	...
	i2c_write8(addr, EXPANDER_REG_IODIRB, 0b11111111);
	i2c_write8(addr, EXPANDER_REG_IODIRA, 0b00000000);
	
	// enable pullups for inputs
	i2c_write8(addr, EXPANDER_REG_GPPUB, 0b11111111);
    i2c_write8(addr, EXPANDER_REG_GPPUA, 0b00000000);
}
...
uint8_t i2c_read_regA_on(uint8_t addr) {
	return i2c_read8(addr, EXPANDER_REG_GPIOB);
}

void i2c_select_regB_on(uint8_t addr, uint8_t index) {
    i2c_write8(addr, EXPANDER_REG_GPIOA, 0xFF & ~(1 << index));
} 

void i2c_unselect_regB_on(uint8_t addr) {
    i2c_write8(addr, EXPANDER_REG_GPIOA, 0xFF & ~(0<<7));
}